...
# --- 页面配置 --- 
st.set_page_config(page_title="SQL Talk Pro", page_icon="✨", layout="wide", initial_sidebar_state="expanded")
...
def get_full_base_url():
    return (st.session_state.agent_base_url_input or (REMOTE_AGENT_URL if st.session_state.use_remote else LOCAL_AGENT_URL)).rstrip("/")
...
async def send_via_http(message: str) -> str:
    ...
    if st.session_state.use_remote and not auth_headers:
        raise RuntimeError("缺少 API Key。请在侧边栏填写或设置环境变量 API_GATEWAY_API_KEY")
    ...
    try:
        async with httpx.AsyncClient(timeout=httpx.Timeout(120, connect=30, read=120, write=30)) as client:
            # Step 1: Create session if not already created
            session_url = f"{base}/apps/{APP_NAME}/users/{USER_ID}/sessions/{session_id}"
            session_resp = await client.post(session_url, headers=auth_headers)
            ...
            async with client.stream("POST", f"{base}/run_sse", json=request_body, headers=auth_headers) as r:
...
with st.sidebar:
    ...
    # 连接模式切换
    st.session_state.use_remote = st.toggle("连接远程 Agent", value=st.session_state.use_remote, help="切换连接本地或远程部署的 Agent 服务")
    
    # Agent 基地址
    default_url = REMOTE_AGENT_URL if st.session_state.use_remote else LOCAL_AGENT_URL
    st.session_state.agent_base_url_input = st.text_input("Agent 基地址", value=st.session_state.agent_base_url_input or default_url)
    ...
    # 请求超时
    st.session_state.a2a_timeout_secs = st.number_input("请求超时秒数", min_value=60, max_value=3600, value=st.session_state.a2a_timeout_secs, step=30, help="长查询建议 600-1200 秒")
